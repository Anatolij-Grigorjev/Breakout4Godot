...
func test_ball_hit_paddle_low_pos_anlge_paddle_fixes():
    ...
    var corrected_ball_angle = rad2deg(Vector2(ball.direction.x, ball.direction.y).angle())
    ...
    assert_ne(corrected_ball_angle, rad2deg(ball.direction.bounce(collision.normal).angle()))


func test_ball_hit_paddle_low_neg_anlge_paddle_fixes():

    var ball_angle = deg2rad(-165)
    var ball_height = 100
    var ball_distance = ball_height / tan(ball_angle)
